...
def _log_ordered_scalars(writer, values, global_step, tag_prefix=None):
    ...
    ordered = [
        ...
        'loss_c', 'loss_c_l1', 'loss_c_msg', 'loss_c_tv', 'loss_c_perceptual', 'loss_c_dssim', 'loss_c_semvar',
...
def compute_targets(predictions, batch):
    ...
    S_c_star = rgb / A_safe
    S_c_star = torch.nan_to_num(S_c_star, nan=0.0, posinf=20.0, neginf=0.0).clamp(0.0, 20.0)
...
def _compute_lmse(pred, target, valid_mask, window_size=20, stride=10, min_valid_ratio=0.5):
    ...
    # 6. Average over valid patches per image
    mse_sum = (mse_per_patch * has_signal).sum(dim=1)
    valid_count = has_signal.sum(dim=1).clamp_min(1)
    return (mse_sum / valid_count).mean()
...
def _masked_scale_invariant_rmse(pred, target, valid_mask, eps=1e-7, shading_cap=None):
    ...
    valid_counts = mask.reshape(pred.shape[0], -1).sum(dim=1).clamp_min(1)
    
    p_mean = p_masked.reshape(pred.shape[0], -1).sum(dim=1) / valid_counts
    t_mean = t_masked.reshape(target.shape[0], -1).sum(dim=1) / valid_counts
    
    p_mean = p_mean.reshape(-1, 1, 1, 1)
    t_mean = t_mean.reshape(-1, 1, 1, 1)
    
    p_norm = p_masked / (p_mean + eps)
    t_norm = t_masked / (t_mean + eps)
    
    diff_sq = ((p_norm - t_norm) ** 2) * mask
