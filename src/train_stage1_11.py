...
TB_TAGS = {
    ...
    's_d_lmse': '03_DiffuseShading_Sd/1_lmse',
    's_d_rmse': '03_DiffuseShading_Sd/2_rmse',
    's_d_ssim': '03_DiffuseShading_Sd/3_ssim',
}


def _log_ordered_scalars(
    writer,
    values,
    global_step,
    log_semvar=True,
    ...
    if log_semvar:
        ordered.append('loss_c_semvar')
    ordered.extend(['loss_d', 'loss_d_mse', 'loss_d_msg', 'loss_d_dssim'])
    if log_recon:
        ordered.append('loss_recon')
    ordered.extend([
...
def compute_targets(predictions, rgb, albedo_raw, valid_mask, illum_raw=None, m_diffuse=None):
    """Compute V11 targets for albedo and diffuse shading only."""
    eps = 1e-6
    ...
    # Use diffuse GT when available and routed; fallback otherwise.
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train Stage 1 V11')
...
def _log_val_examples(writer, global_step, rgb, predictions, targets, max_items=2, full_rgb_list=None, sample_index=None):
    """Log qualitative validation examples for V11 outputs (a_d and pi)."""
    ...
    for i in range(b):
        ...
        # 2. Diffuse shading via inverse-domain visualization (1 - D)
        ...
        blank_tile = torch.full_like(_vis_tonemap(rgb[i:i+1], scale=1.0), 0.93)
        ...
        for name, tile in sample_tiles:
            try:
                ...
                # Add card frame and dark footer band for cleaner readability.
                c, h, w = norm_tile.shape
                canvas = torch.full(
                    (c, h + footer_h + 2 * card_pad, w + 2 * card_pad),
                    0.98,
                    dtype=norm_tile.dtype,
                    device=norm_tile.device,
                )
                canvas[:, card_pad:card_pad + h, card_pad:card_pad + w] = norm_tile
                canvas[:, card_pad + h:, :] = 0.15
                ...
                try:
                    ...
                    x = max(0, int(((w + 2 * card_pad) - text_w) // 2))
                    y = card_pad + h + 10
                    draw.text((x, y), name, fill=(240, 240, 240), font=font)
        ...
        if len(tiles) >= 8:
            row1_tiles = tiles[:4]
            row2_tiles = tiles[4:8]

            sep = torch.full(
                (row1_tiles[0].shape[0], row1_tiles[0].shape[1], gutter),
                1.0,
                dtype=row1_tiles[0].dtype,
                device=row1_tiles[0].device,
            )
            row1_img = torch.cat([row1_tiles[0], sep, row1_tiles[1], sep, row1_tiles[2], sep, row1_tiles[3]], dim=2)
            row2_img = torch.cat([row2_tiles[0], sep, row2_tiles[1], sep, row2_tiles[2], sep, row2_tiles[3]], dim=2)

            hsep = torch.full(
                (row1_img.shape[0], gutter, row1_img.shape[2]),
                1.0,
                dtype=row1_img.dtype,
                device=row1_img.device,
            )
            strip = torch.cat([row1_img, hsep, row2_img], dim=1)
        else:
...
def validate(
    ...
    Args:
        ...
        log_semvar: bool. If False, do not write semvar scalar to TensorBoard.
        log_recon: bool. If False, do not write recon scalar to TensorBoard.
    """
    model.eval()
    total_loss = {}
...
def build_stage1_model(config):
    ...
    if version != 11.0:
        raise ValueError(f"train_stage1_11.py only supports model.version=11, got {version}")
    model_cfg = {
    ...
    return IntrinsicDecompositionV11(model_cfg)
...
def build_optimizer_v11(model, train_cfg, model_cfg):
...
def main():
    ...
    optimizer = build_optimizer_v11(model, config['train'], config['model'])
    ...
    log_semvar = float(config['loss'].get('lambda_semvar', 0.0)) > 0.0
    log_recon = float(config['loss'].get('lambda_recon', 0.0)) > 0.0
    ...
    running = {'loss_c': 0.0, 'loss_d': 0.0, 'loss_recon': 0.0, 'loss_total': 0.0}
    last_phase = None
