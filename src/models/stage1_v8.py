"""Stage 1 V8: CCR-guided DecA, Ag-guided DecB, ConcatProj-guided DecC."""
...
from .modules.concat_projection import ConcatProjection
from .stage1_v5 import compute_ccr


class IntrinsicDecompositionV8(nn.Module):
    def __init__(self, config):
        ...
        # Dec A: Normals + CCR
        self.attn_a_n3 = ResidualAttention(768, 512)
        self.attn_a_ccr3 = ResidualAttention(768, 512)
        self.attn_a_n2 = ResidualAttention(384, 256)
        self.attn_a_ccr2 = ResidualAttention(384, 256)
        self.attn_a_n1 = ResidualAttention(192, 128)
        self.attn_a_ccr1 = ResidualAttention(192, 128)

        # Dec B: FiLM + GuidanceEncoder_B
        self.attn_b3 = ResidualAttention(768, 512)
        self.attn_b2 = ResidualAttention(384, 256)
        self.attn_b1 = ResidualAttention(192, 128)

        # Dec C: CCR + SPADE (except s3) + ConcatProjection
        self.attn_c_ccr3 = ResidualAttention(768, 512)
        self.concat_proj_c3 = ConcatProjection(768, 512)

        self.attn_c_ccr2 = ResidualAttention(384, 256)
        self.spade_c2 = SPADE(num_channels=384, num_classes=num_seg_classes)
        self.concat_proj_c2 = ConcatProjection(384, 256)

        self.attn_c_ccr1 = ResidualAttention(192, 128)
        self.spade_c1 = SPADE(num_channels=192, num_classes=num_seg_classes)
        self.concat_proj_c1 = ConcatProjection(192, 128)

        # Dec D: Normals + GuidanceD
    ...
    def forward(self, rgb, m_diffuse=None, normals=None, seg=None, valid_mask=None, ccr=None, **kwargs):
        ...
        d_g = self.decoder_a(
            ...
            skip_features,
            stage_ops=[
                lambda x: self.attn_a_ccr3(self.attn_a_n3(x, normal_feats[3]), ccr_feats[3]),
                lambda x: self.attn_a_ccr2(self.attn_a_n2(x, normal_feats[2]), ccr_feats[2]),
                lambda x: self.attn_a_ccr1(self.attn_a_n1(x, normal_feats[1]), ccr_feats[1]),
            ],
        )
        ...
        a_d = self.decoder_c(
            ...
            skip_features,
            stage_ops=[
                lambda x: self.concat_proj_c3(self.attn_c_ccr3(x, ccr_feats[3]), g_c[3]),
                lambda x: self.concat_proj_c2(self.spade_c2(self.attn_c_ccr2(x, ccr_feats[2]), seg), g_c[2]),
                lambda x: self.concat_proj_c1(self.spade_c1(self.attn_c_ccr1(x, ccr_feats[1]), seg), g_c[1]),
            ],
        )
