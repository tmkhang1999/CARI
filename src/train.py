...
TB_TAGS = {
    ...
    'loss_c_recon': '1. Losses/C_Recon',
    'loss_ccr_edge': '1. Losses/C_CCR_Edge',
    'loss_d': '1. Losses/D_total',
    'loss_d_mse': '1. Losses/D_MSE',
    'loss_d_msg': '1. Losses/D_MSG',
    'loss_d_dssim': '1. Losses/D_DSSIM',
...
def _log_ordered_scalars(writer, values, global_step, tag_prefix=None):
    ...
    ordered = [
        'loss_total',
        'loss_a', 'loss_a_mse', 'loss_a_msg', 'loss_a_dssim',
        'loss_b', 'loss_b_mse', 'loss_b_msg',
        'loss_c', 'loss_c_l1', 'loss_c_msg', 'loss_c_tv', 'loss_c_perceptual', 'loss_c_dssim', 'loss_c_semvar', 'loss_c_recon', 'loss_ccr_edge',
...
def compute_targets(predictions, batch):
    ...
    # Use raw albedo to maintain the exact physical relationship I = A * S.
    # The scale_match function will naturally compute 'c' to handle any HDR range
    # differences between the physical target and the [0,1] model output.
    ...
    S_c_star = rgb / A_safe
    # Removed upper clamp to allow unbounded shading targets (HDR preservation)
    ...
    iuv_star = rgb_to_iuv(S_c_star)
    D_g_star = iuv_star[:, 0:1]
    xi_star = iuv_star[:, 1:3]
...
def _apply_diffuse_detach(predictions, m_diffuse):
    ...
    mask = m_diffuse.reshape(-1, 1, 1, 1).to(predictions['pi'].device)
...
def _log_val_examples(
    ...
    for i in range(b):
        ...
        # 2. Colorful shading GT/pred via IUV composition
        iuv_gt = torch.cat([targets['D_g_star'][i:i+1], targets['xi_star'][i:i+1]], dim=1)
        s_c_gt_linear = iuv_to_rgb(iuv_gt)

        if 's_c' in predictions:
            s_c_pred_linear = predictions['s_c'][i:i+1]
        elif 'xi' in predictions:
            iuv_pred = torch.cat([predictions['d_g'][i:i+1], predictions['xi'][i:i+1]], dim=1)
            s_c_pred_linear = iuv_to_rgb(iuv_pred)
        else:
        ...
        sample_tiles = [
            ('Input RGB', _gamma_correct(_vis_tonemap(rgb[i:i+1], scale=None, valid_mask=v_mask))), # Auto-exposure for HDR Input
            ('Gray Shading GT', _gamma_correct(s_g_gt_vis)),
            ('Colorful GT', _gamma_correct(s_c_gt_vis)),
            ('Diffuse GT', _gamma_correct(s_d_gt_vis)),
            ('Albedo GT', a_d_gt_vis),
            ('Diffuse Recon GT', _gamma_correct(recon_gt_vis)),

            ('', blank_tile),
            ('Gray Shading Pred', _gamma_correct(s_g_pred_vis)),
            ('Colorful Pred', _gamma_correct(s_c_pred_vis)),
            ('Diffuse Pred', _gamma_correct(s_d_pred_vis)),
            ('Albedo Pred', a_d_pred_vis),
            ('Diffuse Recon Pred', _gamma_correct(recon_vis)),
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(tqdm(dataloader, desc='Validation', total=progress_total)):
            ...
            if compute_val_losses:
                losses = criterion(
                    predictions, targets, m_diffuse, loss_mask, _loss_seg(model, seg),
                    normals=normals, rgb=rgb, ccr=ccr
                )
