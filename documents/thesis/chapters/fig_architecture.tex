...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[
    ...
    \node[thumb] (imgT) at (0,0) {\includegraphics[width=19mm]{images/arch/input.png}};
    ...
    \node[thumb, right=2.5mm of Asym] (albT) {\includegraphics[width=15mm]{images/arch/albedo.png}};
    \node[io, right=8mm of sh] (Ssym) {$\boldsymbol{\pi}\!\in\!(0,1)^3$\\[1pt]$\mathbf{S}_d{=}\tfrac{1-\boldsymbol{\pi}}{\boldsymbol{\pi}}$};
    \node[thumb, right=2.5mm of Ssym] (shaT) {\includegraphics[width=15mm]{images/arch/shading.png}};
    ...
    \node[thumb, right=6mm of R] (resT) {\includegraphics[width=13mm]{images/arch/residual.png}};
    ...
    % ── RGB skip: the only input skip actually enabled (shading has none;
    % v17.yaml sets shading_lum_skip=false, albedo_chroma_skip=false) ───────
    ...
    \begin{scope}[shift={(legY)}]
      ...
      \node[io, right=1mm of l4t, xshift=14mm] (sc){RGB skip};
    \end{scope}
  \end{tikzpicture}}
  \caption[Model architecture and supervision.]{%
    ...
    output heads (amber) read it. The albedo head additionally takes a full-resolution
    \emph{RGB skip}: the gamma-encoded input image, concatenated in directly so the material
    colour DINOv2's illumination-invariant tokens discard is not lost. This skip is safe only
    because $\mathcal{L}_{\text{inv}}$ constrains what it can carry through; without that
    constraint the illuminant colour would flow straight down it into $\mathbf{A}$. The
    shading head has no such skip; it predicts the inverse shading $\boldsymbol{\pi}$ from the
    shared DPT features alone, from which
