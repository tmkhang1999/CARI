...
\begin{figure}[t!]
  \centering
  \includegraphics[width=\textwidth]{images/cari/cari_mechanism.png}
  \caption[The CARI training-time mechanism.]{%
    CARI applies only during training. The same shared-weight model runs twice,
    once per illuminant, on two real photographs of one MID scene; the resulting
    albedo predictions $\mathbf{A}_1$ and $\mathbf{A}_2$ are pushed together by
    $\mathcal{L}_{\text{inv}}$ (\eqnref{eq:linv}). Inference sees only one branch
    of this diagram: a single image through a single forward pass, with no pairing
    and no extra cost.
  }
  \label{fig:cari_mechanism}
\end{figure}
