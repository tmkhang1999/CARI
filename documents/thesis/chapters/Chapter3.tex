...
\input{./chapters/fig_inverse_shading}
...
\input{./chapters/fig_architecture}
...
\figref{fig:cari} at the end of this section shows the complete mechanism, once
both losses below have been defined.
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=\textwidth]{images/wb/wb_lanes.png}
  \caption[Two supervision lanes: a white-balanced anchor and a raw CARI pair.]{%
    The two supervision lanes are built from different pre-processing of the same
    kind of scene, not different data. The absolute anchor (left) is
    white-balanced, so the sofa's material colour is the only signal left to
    supervise --- this is what pins $\mathbf{A}$ to a true colour rather than an
    arbitrary one. The CARI pair (right) is deliberately left raw: both
    observations still carry their illuminant's cast, which is exactly the
    variation $\mathcal{L}_{\text{inv}}$ and $\mathcal{L}_{\text{expl}}$ need to
    see in order to learn what to discard.
  }
  \label{fig:wb_lanes}
\end{figure}
