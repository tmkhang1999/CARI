...
\begin{figure}[t!]
  \centering
  \includegraphics[width=\textwidth]{images/losses/anchor_geometry.png}
  \caption[$\mathcal{L}_{\text{a}}$ versus $\mathcal{L}_{\text{chroma}}$: what each anchor
  actually penalises.]{%
    \eqnref{eq:l_albedo} and \eqnref{eq:l_chroma} differ only in whether the vectors
    are normalised first, but that changes what error each one can see.
    $\mathcal{L}_{\text{a}}$ (left) is the raw distance between the predicted and
    reference albedo vectors, so it conflates hue error with brightness error --- a
    prediction can lower this loss by getting \emph{closer in scale} without getting
    the colour right. $\mathcal{L}_{\text{chroma}}$ (right) divides out the length of
    both vectors before comparing them, so only the angle between them --- the hue ---
    is penalised. This is the same failure this thesis's corrected MID metrics are built
    to catch at evaluation time (\secref{sec:mid_results}): a term that cannot
    distinguish scale from colour will always be satisfiable by discarding colour.
  }
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=\textwidth]{images/data/dataset_roles.jpg}
  \caption[One representative frame per dataset, with its training role.]{%
    Hypersim and InteriorVerse are both synthetic and both supply supervised
    albedo; MID is real and is the only source of the paired cross-illuminant
    observations CARI is trained on. No single dataset provides everything the
    model needs.
  }
