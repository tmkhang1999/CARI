...
single number. A third configuration, \textbf{Ours (IIW-fine-tuned)}, is a diagnostic used only to
measure the effect of direct IIW supervision; it is never treated as a zero-shot result
and is confined to \secref{sec:iiw_results}, where \tabref{tab:iiw_finetune_cost}
collects everything it costs.
...
CRefNet \mycite{luo23crefnet}, the Ordinal Shading stage of CD-IID
\mycite{careaga23ordinal}, the full five-stage CD-IID cascade
\mycite{careaga24cdiid}, and RGB$\rightarrow$X \mycite{zeng24rgbx} locally under our
exact pipeline (same output-space normalisation and splits, identical inference
resolution per benchmark). The last two were integrated after the first submission of
this work; they are evaluated on MID here, and the older baselines are cited from their
publications and appear below the rule in each table. Our own constancy metrics ($C_{\text{mat}}$, $C_{\text{arap}}$) are not reported by
those methods and are marked n/a; a cell marked ``-'' is simply one for which no value
is available.
...
($304$M) is frozen and excluded from the optimiser, so the trainable count is far
lower than for the diffusion baselines ($\sim$1290M) or the five-stage CD-IID
cascade (442.8M, every stage trained), an efficiency the constancy results should
be read against. On \emph{total} parameters the gap is much smaller --- our frozen
backbone puts us at 322.9M, only $1.4\times$ below CD-IID and above CRefNet's
66.6M --- so the saving is specifically in weights that had to be optimised, not in
deployed model size.
...
\begin{sidewaystable}[p!]
  ...
  \begin{tabularx}{\textheight}{@{}
    ...
    CD-IID \mycite{careaga24cdiid}            & Five-stage MiDaS-style cascade
        & 8 synthetic sets + MIDIntrinsics; Hypersim for shading & 442.8 & 442.8 & 0.254 & linear albedo + RGB shading + residual \\
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=0.72\textwidth]{images/losses/metric_card_cmat.png}
  \caption[$C_{\text{mat}}$: four illuminants, one material.]{%
    $C_{\text{mat}}$ is the coefficient of variation of one material's mean
    luminance across the scene's illuminant conditions. A stable prediction gives
    four nearly equal bars; a prediction whose brightness tracks the lamp gives
    four bars with visible spread, which is exactly the quantity the standard
    deviation in \eqnref{eq:cmat} measures.
  }
  \label{fig:metric_card_cmat}
\end{figure}
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=0.72\textwidth]{images/losses/metric_card_castrel.png}
  \caption[$\text{Cast}_{\text{rel}}$: one material's chroma across illuminants.]{%
    Plotting one material's per-illuminant chromaticity $(r_{m,k}, b_{m,k})$ as a
    scatter makes the two failure modes visually distinct: a stable material clusters
    tightly, while a material whose colour tracks the lamp strings out along the
    illuminant-colour direction. $\delta_m$ (\eqnref{eq:castterms}) is the radius of
    that cluster.
  }
  \label{fig:metric_card_castrel}
\end{figure}
...
material matches that reference, not merely its amount of colour.

\begin{figure}[t!]
  \centering
  \includegraphics[width=0.72\textwidth]{images/losses/metric_card_chromaerr.png}
  \caption[$\text{Chroma}_{\text{err}}$: stable is not the same as correct.]{%
    $\text{Cast}_{\text{rel}}$ alone cannot see this failure: a prediction can be
    perfectly clustered (stable) yet sit on entirely the wrong point in chroma space.
    $\text{Chroma}_{\text{err}}$ is the distance from that cluster to the pseudo-GT
    reference, so it is zero only when the stable colour is also the right one.
  }
  \label{fig:metric_card_chromaerr}
\end{figure}
...
This is deliberately an in-domain diagnostic: our method trains on MID, and so does
CD-IID \mycite{careaga24cdiid}. We train on the official MID training split and hold
out its $30$-scene test split, which is the split evaluated throughout this chapter.
CD-IID's description of its training data names the multi-illumination dataset without
stating a split, so we cannot verify which scenes it saw; that is a limit on what any
comparison against it can claim, not an allegation. The table therefore compares
...
\begin{table}[t!]
  \caption[MID constancy and chroma calibration.]{%
    ...
    detector: below $1$ is greyed colour, above $1$ over-saturation. Red, orange,
    and yellow mark first, second, and third numerical rank in each column;
    ...
    reports directly comparable values. The full CD-IID cascade and RGB$\rightarrow$X were
    added to the harness after the first submission of this work and are evaluated here
    under the identical protocol; \tabref{tab:mid_paired} gives the paired tests. $\dagger$ denotes the full
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    \midrule
    Ours (full model)                          & \ranktwo{0.130\,{\scriptsize[.117,.144]}} & \ranktwo{0.121\,{\scriptsize[.102,.143]}} & 0.421\,{\scriptsize[.357,.485]} & \rankthree{0.941} \\
    Ours (base CARI)                           & 0.157\,{\scriptsize[.141,.174]} & \rankthree{0.129\,{\scriptsize[.109,.150]}} & 0.425\,{\scriptsize[.360,.491]} & \rankone{0.999} \\
    CD-IID \mycite{careaga24cdiid}             & 0.190\,{\scriptsize[.172,.210]} & \rankone{0.090\,{\scriptsize[.073,.110]}} & \rankone{0.306\,{\scriptsize[.262,.355]}} & \ranktwo{0.944} \\
    RGB$\rightarrow$X \mycite{zeng24rgbx}      & \rankone{0.128\,{\scriptsize[.115,.141]}} & 0.203\,{\scriptsize[.170,.239]} & \ranktwo{0.338\,{\scriptsize[.273,.409]}} & 0.907 \\
    Marigold-App \mycite{marigoldiid24}        & 0.193\,{\scriptsize[.173,.214]} & 0.195\,{\scriptsize[.152,.245]} & \rankthree{0.355\,{\scriptsize[.309,.406]}} & 0.728 \\
    Marigold-Light \mycite{marigoldiid24}      & 0.546\,{\scriptsize[.462,.645]} & 0.154\,{\scriptsize[.124,.187]} & 0.392\,{\scriptsize[.333,.459]} & 1.148 \\
    CRefNet \mycite{luo23crefnet}              & \rankthree{0.151\,{\scriptsize[.138,.164]}} & 0.201\,{\scriptsize[.165,.241]} & \rankthree{0.355\,{\scriptsize[.309,.406]}} & 0.484 \\
    Ordinal Shading \mycite{careaga23ordinal}  & 0.252\,{\scriptsize[.229,.279]} & 0.148\,{\scriptsize[.130,.166]} & 0.549\,{\scriptsize[.463,.637]} & 0.924 \\
    \bottomrule
  \end{tabular}}
\end{table}

\tabref{tab:mid} states the central result of this chapter, and it is not a victory on any
single column. We report the losses first, because the paired bootstrap of
\tabref{tab:mid_paired} makes them unambiguous.

We do \emph{not} win illuminant invariance. Every external method except Ordinal Shading
attains a lower $\text{Cast}_{\text{rel}}$ than our $0.421$, and the paired difference is
significant for four of the five. CD-IID leads the column outright at $0.306$.

We also do not win colour calibration. On $\text{Chroma}_{\text{err}}$ our full model scores
$0.121$, second to CD-IID's $0.090$, and that gap is significant under the paired test
($+0.032\,[+0.016, +0.049]$). This corrects an earlier reading of this table. When only the
four originally integrated baselines were available, our model led this column over all of
them and the natural explanation was the collapse mechanism below; running the full CD-IID
cascade shows that a method can reach better calibration \emph{and} better invariance without
discarding colour, so that explanation cannot be general.

\begin{table}[t!]
  \caption[Paired comparisons against each external baseline.]{%
    Paired differences on the held-out MID split, our full model minus each external
    baseline, scene by scene. All three metrics are lower-is-better, so a negative
    entry means our model wins. Brackets are $95\%$ percentile-bootstrap intervals
    over the $30$ scenes ($20{,}000$ resamples); \textbf{bold} marks an interval
    clear of zero. Because every method is scored on the same scenes, the difference
    can be taken per scene, which cancels scene difficulty; this is the test behind
    the significance claims in this section, and it resolves comparisons that the
    marginal intervals of \tabref{tab:mid} leave looking ambiguous. Reproduce with
    \texttt{tests/eval/paired\_bootstrap\_mid.py}.
  }
  \label{tab:mid_paired}
  \centering
  \small
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{@{}lccc@{}}
    \toprule
    Ours (full model) $-$ & $C_{\text{mat}}$ & $\text{Chroma}_{\text{err}}$ & $\text{Cast}_{\text{rel}}$ \\
    \midrule
    CD-IID \mycite{careaga24cdiid}               & \textbf{-0.060\,{\scriptsize[-0.073,-0.047]}} & \textbf{+0.032\,{\scriptsize[+0.016,+0.049]}} & \textbf{+0.115\,{\scriptsize[+0.083,+0.149]}} \\
    RGB$\rightarrow$X \mycite{zeng24rgbx}        & +0.002\,{\scriptsize[-0.011,+0.017]} & \textbf{-0.081\,{\scriptsize[-0.112,-0.052]}} & \textbf{+0.083\,{\scriptsize[+0.021,+0.144]}} \\
    CRefNet \mycite{luo23crefnet}                & \textbf{-0.021\,{\scriptsize[-0.029,-0.011]}} & \textbf{-0.079\,{\scriptsize[-0.108,-0.052]}} & \textbf{+0.066\,{\scriptsize[+0.027,+0.113]}} \\
    Marigold-App \mycite{marigoldiid24}          & \textbf{-0.062\,{\scriptsize[-0.081,-0.045]}} & \textbf{-0.074\,{\scriptsize[-0.110,-0.040]}} & \textbf{+0.066\,{\scriptsize[+0.028,+0.105]}} \\
    Marigold-Light \mycite{marigoldiid24}        & \textbf{-0.416\,{\scriptsize[-0.513,-0.334]}} & \textbf{-0.033\,{\scriptsize[-0.065,-0.002]}} & +0.029\,{\scriptsize[-0.023,+0.080]} \\
    Ordinal Shading \mycite{careaga23ordinal}    & \textbf{-0.122\,{\scriptsize[-0.145,-0.103]}} & \textbf{-0.026\,{\scriptsize[-0.044,-0.009]}} & \textbf{-0.128\,{\scriptsize[-0.165,-0.097]}} \\
    \bottomrule
  \end{tabular}}
\end{table}

For \emph{some} of the field, though, the invariance ranking does reflect what a method
discards. CRefNet retains under half the pseudo-GT between-material chroma spread
($\text{Chroma}_{\text{fid}} = 0.484$) and Marigold-App under three quarters ($0.728$),
against our $94.1\%$. A prediction cannot drift in a dimension it has thrown away, so those
two buy part of their $\text{Cast}_{\text{rel}}$ lead by not carrying the colour that could
vary. CD-IID is the counter-example that bounds the argument: it retains $0.944$, statistically
indistinguishable from ours, and still leads both colour columns. Invariance won by
desaturation is a real failure mode, but it is not the only route to invariance.

That the two columns can diverge in more than one way is the strongest evidence in this
chapter for reporting them separately. The eight rows contain three distinct behaviours that a
single fidelity number would conflate. CRefNet is \emph{stable but colourless}: low
$\text{Cast}_{\text{rel}}$, and $\text{Chroma}_{\text{fid}} = 0.484$ because the colour is
gone. RGB$\rightarrow$X is \emph{colourful but miscalibrated}: it keeps the spread
($\text{Chroma}_{\text{fid}} = 0.907$) yet places it on the wrong hues, giving the worst
$\text{Chroma}_{\text{err}}$ in the table ($0.203$, significantly worse than ours). CD-IID is
good on both. A pooled score in the style of \eqnref{eq:castrms} would rank CRefNet well and
say nothing about RGB$\rightarrow$X's error; $\text{Chroma}_{\text{fid}}$ alone would clear
RGB$\rightarrow$X and condemn CRefNet; only the pair separates all three. The decomposition of \eqnref{eq:castdecomp} into \eqnref{eq:castrel} and
\eqnref{eq:chromafid} was argued for on principle; these rows are the measurement that
it is necessary.

What survives is therefore narrow, and we state it at that width. Our model leads
\emph{lightness} stability: $C_{\text{mat}} = 0.130$, significantly better than every row
except RGB$\rightarrow$X, from which it is not separable ($+0.002\,[-0.011, +0.017]$). It is
second on colour calibration and mid-table on hue invariance. Every one of those statements
is a paired test, not a reading of overlap between the marginal intervals of \tabref{tab:mid};
with between-scene spread as large as it is here, that distinction decides most of them.

The efficiency comparison is what makes the surviving claim interesting rather than merely
modest. That lightness-stability result is obtained by training $18.5$M parameters against
CD-IID's $442.8$M --- every stage of which is optimised --- and roughly $1290$M for each
diffusion baseline, at $0.148$\,s per image against $0.254$\,s and $0.8$--$1.0$\,s
(\tabref{tab:methods}). The saving is in weights that had to be optimised, not in deployed
size: our frozen backbone leaves us at $322.9$M total, only $1.4\times$ below CD-IID. Read
together, the two results say that a large part of cross-illuminant stability is reachable by
changing the training \emph{evidence} rather than by scaling the decoder, which is the claim
the ablation of \secref{sec:ablation} tests directly and the one this thesis is actually
about. It is not the best
...
\begin{table}[t!]
  \caption[IIW fine-tuning trade-off.]{%
    What the IIW fine-tuning costs. Our full model versus the same model fine-tuned on
    the IIW training split, across the guards that the WHDR column of
    \tabref{tab:iiw} cannot see. This configuration is a diagnostic, not a candidate
    model, so it is reported here rather than as a row in the MID
    (\tabref{tab:mid}) and MAW (\tabref{tab:maw}) benchmark tables. Every guard
    regresses past the pre-specified $10\%$ no-regression threshold.
  }
  \label{tab:iiw_finetune_cost}
  \centering
  \small
  \begin{tabular}{@{}lccc@{}}
    \toprule
    Metric & Ours (full model) & Ours (IIW-fine-tuned) & Change \\
    \midrule
    IIW WHDR $\downarrow$                       & 0.264 & \textit{0.220} & $-17\%$ \\
    \midrule
    MID $C_{\text{mat}}$ $\downarrow$            & 0.130 & 0.191 & $+47\%$ \\
    MID $\text{Chroma}_{\text{err}}$ $\downarrow$ & 0.121 & 0.134 & $+11\%$ \\
    MID $\text{Cast}_{\text{rel}}$ $\downarrow$   & 0.421 & 0.426 & $+1\%$ \\
    MID $\text{Chroma}_{\text{fid}}$ ($\to 1$)    & 0.941 & 0.978 & --- \\
    MAW $\Delta E$ $\downarrow$                  & 3.981 & 5.163 & $+30\%$ \\
    MAW Int.$\times 100$ $\downarrow$            & 0.624 & 1.057 & $+69\%$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t!]
  \caption[MAW albedo accuracy results.]{%
    ...
    chromaticity is inherited from the input image. Red, orange, and yellow
...
$69\%$ increase in intensity error (\tabref{tab:iiw_finetune_cost}; the configuration is a
diagnostic and so is not carried as a row in \tabref{tab:maw}). The effect is therefore not confined to MID's
...
\begin{table}[t!]
  \caption[ARAP cross-illumination constancy.]{%
    ...
    the score toward zero). $C_{\text{arap}}$ measures cross-illumination albedo variation;
    lower is better. Red, orange, and yellow mark first, second, and third numerical rank in
    each column; displayed ties share a rank. These are internal diagnostics, evaluated
    locally.
  }
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    Model & $C_{\text{arap}}$ $\downarrow$ & $C_{\text{arap}}$ $\downarrow$ \\
    \midrule
    Ours (full model)                          & \rankthree{0.142} & \rankone{0.096} \\
    Ours (base CARI)                           & 0.154 & \rankthree{0.119} \\
    Marigold-App \mycite{marigoldiid24}        & \rankone{0.113} & \ranktwo{0.113} \\
    Marigold-Light \mycite{marigoldiid24}      & 0.340 & 0.340 \\
    CRefNet \mycite{luo23crefnet}              & \ranktwo{0.128} & 0.129 \\
    Ordinal Shading \mycite{careaga23ordinal}  & 0.175 & 0.165 \\
    \bottomrule
  \end{tabular}\\[3pt]
  {\footnotesize The pooled $\text{Cast}_{\text{RMS}}$ of \eqnref{eq:castrms} is deliberately
  not reported here: \secref{par:castrms_correction} shows that an absolute chroma variance is
  minimised just as effectively by a colourless albedo as by an invariant one, so it cannot
  stand as a constancy score. $C_{\text{arap}}$ is likewise a pure constancy score and can be
  minimised by a constant prediction; it must be read beside the dense-GT accuracy guard in
  \tabref{tab:arap_accuracy}.}
\end{table}
...
\begin{table}[t!]
  \caption[ARAP dense-albedo accuracy.]{%
    ...
    Table~1, and reports no SSIM because that protocol is unspecified (see the
    note below the table). Red, orange, and yellow mark first, second, and third
    place; LMSE and mn-RMSE rank across all rows, SSIM within the local block.
  }
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    \midrule
    Ours (full model)                          & 0.0275 & \ranktwo{0.2150} & 0.599 \\
    Ours (base CARI)                           & 0.0279 & \rankthree{0.2259} & \ranktwo{0.611} \\
    Marigold-App \mycite{marigoldiid24}        & 0.0266 & 0.2529 & \rankthree{0.607} \\
    Marigold-Light \mycite{marigoldiid24}      & 0.0706 & 0.6187 & 0.589 \\
    CRefNet \mycite{luo23crefnet}              & 0.0223 & 0.2916 & \rankone{0.622} \\
    \midrule
    \multicolumn{4}{@{}l}{\itshape\small Published original-ARAP protocol (Ordinal Shading, Table~1):}\\
    Chromaticity \mycite{arap}                 & 0.024 & 0.296 & --- \\
    NIID-Net \mycite{luo20niid}                & \rankthree{0.022} & \rankone{0.206} & --- \\
    Li \& Snavely \mycite{li18bigtime}         & \rankone{0.018} & 0.227 & --- \\
    Bell et al.\ \mycite{bell14iiw}            & 0.028 & 0.317 & --- \\
    PIE-Net \mycite{das22pienet}               & 0.026 & 0.273 & --- \\
    Ordinal Shading \mycite{careaga23ordinal}  & \ranktwo{0.021} & 0.252 & --- \\
    \bottomrule
  \end{tabular}

  \vspace{0.6em}
  {\footnotesize\raggedright
  \textbf{Why no published SSIM.} We transcribe a published metric only when its
  protocol is specified well enough for the number to mean the same thing as ours.
  That test fails for SSIM: Ordinal Shading's supplement fixes only the invalid-pixel
  threshold (albedo $<0.004$, \S A.6/B.1) and lists SSIM among its ``scale-invariant
  metrics'' without stating the alignment, and the shared \texttt{chrislib} benchmark
  library implements LMSE, si-RMSE and WHDR but no SSIM at all. Running Ordinal
  Shading itself through our closest reconstruction yields $0.671$ against the $0.761$
  it reports, so the gap is a protocol difference we cannot resolve rather than a
  quality difference. The local SSIM column is still internally valid: all five rows
  share one harness.\par}
\end{table}
...
\begin{table}[p]
  ...
  \begin{tabular}{@{}lllcccccc@{}}
    \toprule
    & & & \multicolumn{4}{c}{MID (30 scenes)} & ARAP all & ARAP indoor \\
    \cmidrule(lr){4-7}\cmidrule(lr){8-8}\cmidrule(l){9-9}
    Row & Cross-render & Colour path
        ...
        & $C_{\text{arap}}$ $\downarrow$ & $C_{\text{arap}}$ $\downarrow$ \\
    \midrule
    1 & off & off & 0.250 & 0.477 & 0.186 & 0.512 & 0.164 & 0.122 \\
    2 & on  & off & 0.180 & 0.447 & 0.201 & 0.484 & \textbf{0.145} & \textbf{0.108} \\
    3 & off & on  & 0.259 & 0.444 & \textbf{0.124} & 0.925 & 0.155 & 0.117 \\
    4 & on  & on  & \textbf{0.157} & \textbf{0.425} & 0.129 & \textbf{0.999} & 0.154 & 0.119 \\
    \bottomrule
  \end{tabular}}\\[3pt]
  {\footnotesize MID spread ratios use the ratio of aggregate predicted and pseudo-GT
  spreads, not the biased mean of per-scene ratios. The pooled ARAP
  $\text{Cast}_{\text{RMS}}$ columns are omitted for the reason given under
  \tabref{tab:arap_constancy}: an absolute chroma variance is minimised as effectively by a
  colourless albedo as by an invariant one. The MID fidelity metrics cannot replace them on
  ARAP, which ships no material segmentation, so ARAP contributes constancy
  ($C_{\text{arap}}$) and the dense-GT accuracy of Part~2a instead.}\\[4pt]
  ...
  \begin{tabular}{@{}lllcccccc@{}}
    ...
    \midrule
    1 & off & off & 4.434 & 0.409 & 0.0307 & 0.2378 & 0.602 & 0.334 \\
    2 & on  & off & 4.631 & 0.474 & 0.0320 & 0.2313 & 0.589 & \textbf{0.283} \\
    3 & off & on  & \textbf{4.115} & \textbf{0.395} & 0.0283 & 0.2348 & \textbf{0.617} & 0.327 \\
    4 & on  & on  & 4.155 & 0.504 & \textbf{0.0279} & \textbf{0.2259} & 0.611 & 0.286 \\
    \bottomrule
  \end{tabular}}\\[5pt]
...
\begin{table}[t!]
  ...
  \begin{tabular}{@{}lllcccccccc@{}}
    \toprule
    & & & \multicolumn{3}{c}{MID} & ARAP indoor
        & \multicolumn{2}{c}{MAW} & \multicolumn{2}{c}{Guards} \\
    \cmidrule(lr){4-6}\cmidrule(lr){7-7}\cmidrule(lr){8-9}\cmidrule(l){10-11}
    Row & Configuration & Levers
        & $C_{\text{mat}}$ $\downarrow$ & $\text{Cast}_{\text{rel}}$ $\downarrow$
        & $\text{Chroma}_{\text{err}}$ $\downarrow$
        & $C_{\text{arap}}$ $\downarrow$
        & $\Delta E$ $\downarrow$ & Int.$\times$100 $\downarrow$
        & mn-RMSE $\downarrow$ & WHDR $\downarrow$ \\
    \midrule
    1 & Control (extra training only) & --                      & 0.161 & 0.415 & \textbf{0.115} & 0.116 & 4.179 & \textbf{0.511} & 0.2194 & 0.283 \\
    2 & \quad + levers                & flat, sh\_inv           & 0.146 & 0.426 & \textbf{0.115} & 0.100 & 4.193 & 0.732 & \textbf{0.2099} & 0.281 \\
    3 & \quad + 3D-Front-IID          & --                      & 0.163 & 0.418 & 0.120 & 0.111 & 4.040 & 0.512 & 0.2274 & 0.280 \\
    4 & \quad + 3D-Front + levers     & flat (reduced), sh\_inv & \textbf{0.130} & 0.421 & 0.121 & \textbf{0.096} & \textbf{3.981} & 0.624 & 0.2150 & \textbf{0.264} \\
    5 & \quad + 3D-Front + levers     & flat (full), sh\_inv    & 0.149 & 0.421 & 0.143 & 0.097 & 4.231 & 0.719 & 0.2103 & 0.282 \\
    \bottomrule
  \end{tabular}}\\[4pt]
...
($\Delta E=3.981$), coloured-illuminant ARAP constancy ($C_{\text{arap}}=0.096$), and IIW ordering
